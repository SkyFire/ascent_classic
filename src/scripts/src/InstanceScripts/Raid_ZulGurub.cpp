...
public:
    ...
    JeklikAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
        ...
        spells[0].info = sSpellStore.LookupEntry(TRANSFORM_BAT);
		...
		spells[1].info = sSpellStore.LookupEntry(CRUSHING_BLOW);
		spells[1].targettype = TARGET_ATTACKING;
		spells[1].instant = false;
		spells[1].perctrigger = 10.0f;
		spells[1].attackstoptimer = 2000; // 1sec

		spells[2].info = sSpellStore.LookupEntry(CRUSHING_AOE_SILENCE);
		...
		// 2 phase spells
		spells[3].info = sSpellStore.LookupEntry(MIND_FLAY);
		spells[3].targettype = TARGET_ATTACKING;
		spells[3].instant = false;
		spells[3].perctrigger = 10.0f;
		spells[3].attackstoptimer = 6000; // 1sec

		spells[4].info = sSpellStore.LookupEntry(SHADOW_WORD_PAIN);
		spells[4].targettype = TARGET_ATTACKING;
		spells[4].instant = false;
		spells[4].perctrigger = 10.0f;
		spells[4].attackstoptimer = 2000; // 1sec

		spells[5].info = sSpellStore.LookupEntry(GREAT_HEAL);
...
public:
    ...
    VenoxisAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
        ...
        spells[0].info = sSpellStore.LookupEntry(TRANSFORM_SNAKE);
		spells[0].targettype = TARGET_SELF;
		...
		spells[1].info = sSpellStore.LookupEntry(HOLY_NOVA);
		spells[1].targettype = TARGET_VARIOUS;
		spells[1].instant = false;
		spells[1].perctrigger = 10.0f;
		spells[1].attackstoptimer = 2000; // 2sec

		spells[2].info = sSpellStore.LookupEntry(HOLY_FIRE);
		spells[2].targettype = TARGET_VARIOUS;
		spells[2].instant = false;
		spells[2].perctrigger = 10.0f;
		spells[2].attackstoptimer = 2000; // 2sec

		spells[3].info = sSpellStore.LookupEntry(SPIT_POISON);
